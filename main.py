...
async def watch_order_book_for_pair_robust(exchange, pair, ex_id):
    while True:
        try:
            ...
            if 'exchange_instance' in locals() and exchange_instance.has['close']:
...
async def main():
    ...
    # Criação das tarefas de verificação de arbitragem e do loop de eventos
    # watch_all_exchanges() agora é robusto com reconexão
    # check_arbitrage_opportunities() é mais rápido
    await asyncio.gather(
        watch_all_exchanges(),
        check_arbitrage_opportunities(app),
        app.run_polling()
    )
